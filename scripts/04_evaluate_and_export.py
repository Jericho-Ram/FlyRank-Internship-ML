...
> **Run provenance.** Generated by `scripts/04_evaluate_and_export.py`.
> Scored population excludes the `3,388` of `30,000` rows with
> `impressions_prev_30d == 0`, leaving `{len(final_frame):,}` rows scored,
> since pages with no prior impressions cannot decline.
> Earlier versions published a Precision@50 of `0.680` over all `30,000`
> rows; that figure is withdrawn. See `outputs/known_failure_modes.md`,
> entries 1 and 4. Models are selected by ROC AUC, not Precision@50
> (entry 6). Quote ROC AUC alongside Precision@50, never Precision@50 alone.
